...
A Python project using Office of Rail and Road station-usage data. The pipeline:

- Downloads a CSV and keeps an unchanged copy of the original data.
- Cleans the data with pandas and leaves missing measurements as missing.
- Reshapes ticket-type columns into rows for easier comparisons.
- Checks the data and saves two processed CSVs.
- Stops with an error if a data check fails.
...
- Stopped and disabled SSH with `systemctl` when it was no longer needed, then scanned again to check that port 22 was no longer open.
